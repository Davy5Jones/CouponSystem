...
public class CompaniesDBDAO implements CompaniesDAO {
    ...
    private static final String IS_EXISTS_QUERY = QueryBuilder.select().count(1).as("present").from("`java-151-cs1`.`companies`").where("EMAIL", "=", "?").and().where("password", "=", "?").getQuery();
    ...
    private static final String DROP_COMPANY_QUERY = QueryBuilder.delete().from("`java-151-cs1`.`companies`").where("id", "=", "?").getQuery();
    private static final String GET_COUPONS_BY_COMPANY_QUERY = QueryBuilder.select("*").from("java-151-cs1.coupons").where("COMPANY_ID", "=", "?").getQuery();
    private static final String GET_ALL_COMPANIES_QUERY = "SELECT * FROM `java-151-cs1`.companies;";
    private static final String GET_COMPANY_INSTANCE_QUERY = QueryBuilder.select("*").from("`java-151-cs1`.`companies`").where("id", "=", "?").getQuery(); //"SELECT * FROM `java-151-cs1`.companies where id = ? ;";
    ...
    public void update(Company company, Integer id) throws SQLException, SystemException {
        if (!isExistByNameOrEmail("", company.getName())) {
            throw new SystemException(SystemExceptionEnum.COMPANY_NOT_FOUND.getMessage());
        }
        Map<Integer, Object> map = new HashMap<>();
    ...
    public Company getOne(Integer id) throws SQLException, SystemException {
        ...
        if (company.isEmpty()) throw new SystemException(SystemExceptionEnum.COMPANY_NOT_FOUND.getMessage());
